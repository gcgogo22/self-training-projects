...
func main() {
	...
	http.HandleFunc("/homt.html", homeHandler)
...
func homeHandler(w http.ResponseWriter, r *http.Request) {
	// Check if the login cookies exists
	cookie, err := r.Cookie("user")
	if err != nil || cookie.Value == "" {
		// Cookie does not exist or it's empty, redirect to the login page
...
func verifyHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == "POST" {
		...
		if valid {
			...
			expiration := time.Now().UTC().Add(24 * time.Hour) // Set the cookie expiration
			...
			fmt.Fprintf(w, "Aunthentication failed!")
			return
		}
	}
